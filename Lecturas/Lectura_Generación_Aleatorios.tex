...
\section{Resumen: del LCG a la Variada}

El recorrido de esta lectura puede sintetizarse en tres pasos sucesivos que forman la columna vertebral de cualquier generador estocástico:

\begin{center}
\small
\begin{tabular}{clp{6cm}}
\toprule
\textbf{Paso} & \textbf{Operación} & \textbf{Resultado} \\
\midrule
1 & LCG: $Z_i = (aZ_{i-1}+c) \bmod m$ & Entero pseudoaleatorio $Z_i \in \{0,\ldots,m-1\}$ \\
2 & Normalización: $R_i = Z_i/m$ & Número uniforme $R_i \in (0,1)$ \\
3 & Transformada inversa: $X_i = F^{-1}(R_i)$ & Variada con distribución $F$ deseada \\
\bottomrule
\end{tabular}
\end{center}

En la práctica, librerías como \texttt{numpy} implementan generadores mucho más sofisticados (Mersenne Twister, con período $2^{19937}-1$) y transformadas inversas para decenas de distribuciones. Sin embargo, comprender el LCG y la Transformada Inversa permite entender \emph{por qué} estas herramientas funcionan y cómo diagnosticar sus limitaciones. La implementación computacional completa de estos conceptos se trabaja en el notebook correspondiente del curso.
...
\begin{enumerate}[label={[\arabic*]}]
    ...
    \item Banks, J., Carson, J., Nelson, B., \& Nicol, D. (2014). \textit{Discrete-Event System Simulation}, 5th ed. Pearson. Cap.~8.
    \item Knuth, D.\,E. (1998). \textit{The Art of Computer Programming, Vol.~2: Seminumerical Algorithms}, 3rd ed. Addison-Wesley. Cap.~3.
    \item Ross, S.\,M. (2006). \textit{Simulation}, 4th ed. Academic Press. Cap.~5.
