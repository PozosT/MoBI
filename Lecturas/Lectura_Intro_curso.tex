...
\title{\textbf{Lectura 1: Introducción al Modelamiento y Simulación bajo Incertidumbre}}
...
\begin{lstlisting}[language=Python, caption=Simulación de Utilidad en Python]
import numpy as np
...
# Parametros de la demanda (Incertidumbre)
media_demanda = 600
desv_std_demanda = 100
num_simulaciones = 1000000

# Generacion de escenarios de demanda (Numeros aleatorios)
np.random.seed(42) # Para reproducibilidad
demandas_simuladas = np.random.normal(media_demanda, desv_std_demanda, num_simulaciones)

# Calculo de utilidades para cada escenario
# Utilidad = (Margen * Demanda) - Costo Fijo
utilidades = ((precio_venta - costo_variable) * demandas_simuladas) - costo_fijo

# Analisis de resultados
utilidad_promedio = np.mean(utilidades)
probabilidad_perdida = np.sum(utilidades < 0) / num_simulaciones

print(f"Resultados tras {num_simulaciones} simulaciones:")
print(f"Utilidad Promedio Esperada: ${utilidad_promedio:.2f}")
print(f"Probabilidad de Perdida: {probabilidad_perdida * 100:.2f}%")

# Resultado teorico para validacion:
# Punto de equilibrio = 10000 / (50-30) = 500 unidades
# Probabilidad de demanda < 500 en N(600, 100) es aprox 15.8%
\end{lstlisting}
