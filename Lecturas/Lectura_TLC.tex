...
% Configuración del estilo para los listados de código Python
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{El Teorema del Límite Central y la Simulación}
\fancyfoot[C]{\thepage}
...
\begin{lstlisting}[language=Python, caption=Demostración del TLC con la media de tiradas de un dado.]
import numpy as np
import matplotlib.pyplot as plt

def demostrar_tlc(n_dados, n_experimentos=10000):
    """
    Simula n_experimentos. En cada experimento, se lanzan n_dados
    y se calcula la media. Devuelve un array con todas las medias.
    """
    medias_muestrales = []
    for _ in range(n_experimentos):
        tiradas = np.random.randint(1, 7, size=n_dados)
        media_actual = np.mean(tiradas)
        medias_muestrales.append(media_actual)
    return np.array(medias_muestrales)

# Configurar el grafico
fig, axs = plt.subplots(2, 2, figsize=(12, 10))
fig.suptitle('Demostracion del Teorema del Limite Central', fontsize=16)

# Caso 1: n = 1 (Distribucion original)
medias_n1 = demostrar_tlc(n_dados=1)
axs[0, 0].hist(medias_n1, bins=np.arange(0.5, 7.5, 1), density=True, rwidth=0.8)
axs[0, 0].set_title('Distribucion de la Media de n=1 Dado (Uniforme)')
axs[0, 0].set_ylabel('Densidad')

# Caso 2: n = 2
medias_n2 = demostrar_tlc(n_dados=2)
axs[0, 1].hist(medias_n2, bins=30, density=True)
axs[0, 1].set_title('Distribucion de la Media de n=2 Dados (Triangular)')

# Caso 3: n = 5
medias_n5 = demostrar_tlc(n_dados=5)
axs[1, 0].hist(medias_n5, bins=30, density=True)
axs[1, 0].set_title('Distribucion de la Media de n=5 Dados')
axs[1, 0].set_ylabel('Densidad')

# Caso 4: n = 30
medias_n30 = demostrar_tlc(n_dados=30)
axs[1, 1].hist(medias_n30, bins=30, density=True)
axs[1, 1].set_title('Distribucion de la Media de n=30 Dados (Aprox. Normal)')

plt.tight_layout(rect=[0, 0, 1, 0.96])
# Para ejecutar esto, se necesita matplotlib: pip install matplotlib
# plt.show() # Descomentar para mostrar el grafico
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Construcción de un intervalo de confianza a partir de réplicas.]
import numpy as np
from scipy import stats

def simular_replica_mm1(tasa_llegada, tasa_servicio, max_clientes):
    """Simula una replica de una cola M/M/1 y devuelve el tiempo de espera promedio."""
    reloj = 0.0
    num_llegadas = 0
    num_salidas = 0
    tiempo_llegada = {0: 0} # Diccionario para registrar tiempo de llegada de cada cliente
    tiempos_de_espera = []
    
    tiempo_proxima_llegada = np.random.exponential(1.0 / tasa_llegada)
    tiempo_proxima_salida = float('inf')
    
    while num_salidas < max_clientes:
        if tiempo_proxima_llegada < tiempo_proxima_salida:
            reloj = tiempo_proxima_llegada
            num_llegadas += 1
            tiempo_llegada[num_llegadas] = reloj
            
            if num_llegadas - num_salidas == 1: # Servidor estaba libre
                tiempo_servicio = np.random.exponential(1.0 / tasa_servicio)
                tiempo_proxima_salida = reloj + tiempo_servicio
                tiempos_de_espera.append(0) # Cliente no espero
            
            tiempo_proxima_llegada = reloj + np.random.exponential(1.0 / tasa_llegada)
        else:
            reloj = tiempo_proxima_salida
            num_salidas += 1
            
            if num_llegadas > num_salidas: # Hay alguien en cola
                tiempo_servicio = np.random.exponential(1.0 / tasa_servicio)
                tiempo_proxima_salida = reloj + tiempo_servicio
                
                # Calcular espera del cliente que acaba de entrar en servicio
                espera = reloj - tiempo_llegada[num_salidas + 1]
                tiempos_de_espera.append(espera)
            else:
                tiempo_proxima_salida = float('inf')
                
    return np.mean(tiempos_de_espera)

# --- Parametros del estudio de simulacion ---
N_REPLICAS = 40
TASA_LLEGADA = 0.8
TASA_SERVICIO = 1.0

# --- Ejecutar las replicas ---
resultados_replicas = []
for i in range(N_REPLICAS):
    # Usar una semilla diferente para cada replica
    np.random.seed(i)
    tiempo_espera_promedio = simular_replica_mm1(TASA_LLEGADA, TASA_SERVICIO, 
                                                max_clientes=1000)
    resultados_replicas.append(tiempo_espera_promedio)

# --- Calcular el intervalo de confianza ---
media_muestral = np.mean(resultados_replicas)
std_muestral = np.std(resultados_replicas, ddof=1) # ddof=1 para S
error_estandar = std_muestral / np.sqrt(N_REPLICAS)

# Para un 95% de confianza
nivel_confianza = 0.95
alpha = 1 - nivel_confianza
z_critico = stats.norm.ppf(1 - alpha / 2) # Aprox. 1.96

limite_inferior = media_muestral - z_critico * error_estandar
limite_superior = media_muestral + z_critico * error_estandar

# --- Reportar resultados ---
Wq_teorico = TASA_LLEGADA / (TASA_SERVICIO * (TASA_SERVICIO - TASA_LLEGADA))
print(f"Tiempo de espera promedio teorico (Wq): {Wq_teorico:.4f}")
print(f"Media muestral de las replicas: {media_muestral:.4f}")
print(f"Intervalo de confianza del {nivel_confianza:.0%}: [{limite_inferior:.4f}, {limite_superior:.4f}]")
\end{lstlisting}
