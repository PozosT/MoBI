...
%================================================
% TEMA Y ESTILO (mismo look que tu intro)
%================================================
...
% METADATOS
%================================================
\title[]{Simulación Monte Carlo: estimación, error y distribución de salida}
...
np.random.seed(0)
N = 2000
p = 0.62  # "verdadero" para la simulacion

X = (np.random.rand(N) < p).astype(int)

p_hat = X.mean()
s_hat = X.std(ddof=1)
half  = 1.96 * s_hat / np.sqrt(N)

print(f"p_hat = {p_hat:.4f}  IC95%: [{p_hat-half:.4f}, {p_hat+half:.4f}]")
\end{lstlisting}
...
D = np.clip(D, 0.0, None)           # demanda no negativa
Y = cs * np.maximum(D - Q, 0.0)     # costo por faltantes
...
# (2) Probabilidad de "costo alto"
tau = 200.0
pi_hat = np.mean(Y > tau)

# (3) Cuantil (p90)
q90 = np.quantile(Y, 0.90)

print("E[Y] =", mu_hat, " IC95% +/-", half)
print("P(Y>tau) =", pi_hat, " tau =", tau)
print("q0.90(Y) =", q90)
\end{lstlisting}
...
plt.title("Distribucion empirica de Y (costo por faltantes)")
plt.xlabel("Y"); plt.ylabel("Frecuencia")
plt.show()

# Convergencia de la media corrida
running_mean = np.cumsum(Y) / np.arange(1, len(Y)+1)
plt.figure(figsize=(9,4))
plt.plot(running_mean)
plt.title("Convergencia de la media corrida")
plt.xlabel("n"); plt.ylabel("mu_hat(n)")
plt.show()
\end{lstlisting}
\end{frame}
...
# 1) Generar X_i ~ Bernoulli(p_true) usando Uniforme(0,1)
U = np.random.rand(N)
X = (U < ________).astype(int)

# 2) Estimador de p
p_hat = ________.mean()

# 3) Desviacion muestral (ddof=1) y semi-ancho del IC (CLT)
s_hat = X.std(ddof=______)
half  = 1.96 * s_hat / np.sqrt(______)

print("p_hat =", p_hat)
print("IC95% = [", p_hat-half, ",", p_hat+half, "]")
\end{lstlisting}
...
# (b) Probabilidad de exceder un umbral
tau = 200.0
p_hat = np.mean(Y > tau)
...
# 2) Convergencia de la media corrida
running_mean = np.cumsum(Y) / np.arange(1, len(Y)+1)
