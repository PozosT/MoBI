\documentclass[aspectratio=169, 9 pt]{beamer}
...
\usepackage{hyperref} % Beamer suele cargarlo, pero es seguro dejarlo
\usepackage{qrcode}   % Para poner el QR del video


%================================================
% TEMA Y ESTILO (sobrio y consistente)
%================================================
...
% METADATOS
%================================================
\title[]{Pensamiento Sistémico e Introducción a la Simulación}
\author{Alejandra Tabares Pozos}
\institute[Uniandes]{Universidad de los Andes \\ Departamento de Ingeniería Industrial}
\date{\today}
...
# Normal estandar: pdf y cdf (sin SciPy, autocontenido)
def phi(z):
...
# ---- Calculo analitico rapido: E[min(D,Q)] ----
z = (Q - mu)/sigma
E_sales = mu*Phi(z) - sigma*phi(z) + Q*(1.0 - Phi(z))
E_profit = p*E_sales - c*Q

print("E[ventas] analitico:", E_sales)
print("E[utilidad] analitico:", E_profit)
\end{lstlisting}
\end{frame}
...
# Si se requiere factibilidad no-negativa:
# (El recorte afecta poco porque P(D<0) es muy pequena con mu=100,sigma=30)
...
# IC 95% por CLT: mean +/- 1.96 * s/sqrt(N)
half_width = 1.96 * profit_std / np.sqrt(N)

prob_loss = np.mean(profit < 0.0)

print(f"MC mean: {profit_mean:.2f}  (IC95% +/- {half_width:.2f})")
print(f"Prob(perder dinero): {prob_loss:.4f}")

# Comparacion con "mundo sin incertidumbre" (D=mu, Q=mu)
profit_no_uncertainty = (p - c) * mu
print(f"Utilidad sin incertidumbre (benchmark): {profit_no_uncertainty:.2f}")
print(f"Gap vs MC mean: {profit_no_uncertainty - profit_mean:.2f}")
\end{lstlisting}
\end{frame}
...
plt.hist(profit, bins=40, alpha=0.8, edgecolor='black')
plt.axvline(profit_mean, linestyle='dashed', linewidth=2,
            label=f'Media MC: {profit_mean:.0f}')
plt.axvline(E_profit, linestyle='dashed', linewidth=2,
            label=f'Media analitica: {E_profit:.0f}')
plt.axvline((p-c)*mu, linestyle='dashed', linewidth=2,
            label='Benchmark sin incertidumbre: 3000')
plt.title('Distribucion de utilidades (Q=100)')
plt.xlabel('Utilidad')
...
# (1) Genere demanda D (escoja UNA opcion):
# Opcion 1: dos escenarios (baja/alta)
# D = np.random.choice([80, 120], size=N, p=[0.7, 0.3])

# Opcion 2: normal truncada
mu, sigma = 100, 30
D = np.random.normal(mu, sigma, N)
D = np.clip(D, 0, None)  # demanda no negativa

# (2) Calcule costo por escenario:
Y = cs * np.maximum(D - Q, 0)

# (3) Estime el costo esperado:
print("E[costo] aprox =", Y.mean())
\end{lstlisting}
