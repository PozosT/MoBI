...
% ── Anderson-Darling: fórmula e intuición ────────────────────
\begin{frame}{Anderson--Darling — ¿qué es?}
  \begin{teoremabox}[Estadístico de Anderson--Darling]
    \[
      A^2 \;=\; -n \;-\; \frac{1}{n}\sum_{i=1}^{n}(2i-1)
      \Bigl[\ln F_0(X_{(i)}) \;+\; \ln(1-F_0(X_{(n+1-i)}))\Bigr]
    \]
  \end{teoremabox}

  \textbf{¿Por qué esta forma?}

  Anderson--Darling pondera la diferencia cuadrática entre la empírica y la teórica por
  $\frac{1}{F_0(x)[1-F_0(x)]}$, un peso que \emph{explota en las colas}:

  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[width=9cm, height=3.6cm,
        xlabel={$F_0(x)$}, ylabel={peso},
        xmin=0.01, xmax=0.99, ymin=0, ymax=12,
        domain=0.01:0.99, samples=200]
        \addplot[thick, mobiblue] {1/(x*(1-x))};
        \node[above, text=mobiblue, font=\scriptsize] at (axis cs:0.5,4) {$w(x)=1/[F_0(1-F_0)]$};
      \end{axis}
    \end{tikzpicture}
  \end{center}

  \small Resultado: Anderson--Darling es \emph{sensible a colas}, donde Kolmogorov--Smirnov
  pierde potencia. Los valores críticos dependen de la distribución bajo $H_0$
  (5\%: exponencial 1{,}32; normal/log-normal 0{,}75; Weibull 0{,}76).
\end{frame}
