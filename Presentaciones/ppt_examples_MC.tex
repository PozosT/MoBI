...
\newcommand{\q}{q} % cuantiles

%================================================
% METADATOS
%================================================
\title[]{Monte Carlo en práctica: distribuciones, réplicas, salida y TLC\\(con ejemplos financieros e inventario)}
...
\begin{block}{Esqueleto (Python)}
\begin{lstlisting}[language=Python]
D = np.random.gamma(k, theta, N)
sales = np.minimum(D, Q)
profit = p*sales + v*(Q-sales) - c*Q
mu_hat = profit.mean()
pi_hat = np.mean(D > Q)
\end{lstlisting}
\end{block}
...
\begin{block}{Esqueleto (Python)}
\begin{lstlisting}[language=Python]
N = np.random.poisson(lam, size=Nsim)
Y = np.array([np.random.lognormal(mu,sigma,k).sum() if k>0 else 0.0 for k in N])
theta_hat = np.mean(np.maximum(Y-d,0.0))
pi_hat    = np.mean(Y > d)
\end{lstlisting}
\end{block}
\end{frame}
